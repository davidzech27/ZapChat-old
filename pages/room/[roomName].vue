...
function sendMessage(topicNumber) {
	if (topicText["topic" + topicNumber] !== "") {
		room.sendMessage({ topicNumber: topicNumber, text: topicText["topic" + topicNumber] }, profile.value.username)
	}
...
function viewProfile(message) {
	viewedProfile.value = { username: message.sentBy, email: message.email, bio: message.bio }

	viewProfilePopup.value = true
}
</script>
...
<template>
	<div v-if="room.isLoaded && room.exists" class="grid grid-cols-3">
		...
		<button @click="editRoomPopup = true" v-if="profile.username === roomData.administrator.username" class="absolute cursor-pointer top-4 right-3 py-2 px-4 rounded-full bg-surface bg-opacity-15 hover:bg-opacity-10 z-10">Edit room</button>

		<div v-show="editRoomPopup" @click="editRoomPopup = false" class="absolute h-screen w-screen bg-background bg-opacity-75 backdrop-filter backdrop-blur-md flex justify-center items-center z-10">
			<div @click.stop class="w-1/3 h-2/3 bg-background rounded-2xl border border-white border-opacity-20 grid grid-col pt-32 justify-center">
				<input type="text" placeholder="Topic 1" v-model="topic1" class="w-84 h-16 text-field bg-surface" />
				<input type="text" placeholder="Topic 2" v-model="topic2" class="w-84 h-16 text-field bg-surface" />
				<input type="text" placeholder="Topic 3" v-model="topic3" class="w-84 h-16 text-field bg-surface" />
				<button @click="updateRoom" class="w-84 h-24 mt-6 mb-10 rounded-xl bg-gradient-to-tr from-blue to-purple text-4xl tracking-wider transition-opacity duration-200 hover:opacity-85">Update</button>
			</div>
		</div>
		<div v-show="viewProfilePopup" @click="viewProfilePopup = false" class="absolute h-screen w-screen bg-background bg-opacity-75 backdrop-filter backdrop-blur-md flex justify-center items-center z-10">
			<div @click.stop class="w-1/3 h-2/3 bg-background rounded-2xl border border-white border-opacity-20 grid grid-col pt-32 justify-center text-center items-start">
				<span class="text-2xl">{{ viewedProfile }}</span>
			</div>
		</div>
	</div>
	<div v-else-if="!room.exists" class="h-screen flex justify-center pt-72 px-24 text-9xl text-text font-light tracking-wide">Room doesn't exist yet.</div>
	<div v-else class="gradient h-screen flex justify-center pt-72 text-9xl text-text font-light tracking-wide">
